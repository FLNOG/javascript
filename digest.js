...
var passtable="â‚¬â€šÆ’â€žâ€¦â€ â€¡Ë†â€°Å â€¹Å’Å½â€˜â€™â€œâ€â€¢â€“â€”Ëœâ„¢Å¡â€ºÅ“Å¾Å¸";
...
function validateADSearchBase(data)
{
	var errID = null;
	if (data == null) {
		errID = "ID_WARNING_INV_AD_SEARCHBASE";
	} else if (!checkCtrlChar(data)) {
		errID = "ID_WARNING_INV_AD_SEARCHBASE_STR";
	} else if (data.length > 128 || !checkInvalidString(data)) {
		errID = "ID_WARNING_INV_AD_SEARCHBASE";
	}
	return errID;
...
function hexhash(data_in, code){
	...
	if(code == "SDL"){
		...
		/* ï¿½Ç‚ï¿½ï¿½ï¿½Å‚ï¿½ï¿½È‚ï¿½ï¿½Æ‚ï¿½ï¿½ÍŽbï¿½ï¿½Iï¿½ï¿½SDL */
		data = utf8_to_ISO(data_in);
	}
...
function checkIpv4AddressFormat(data)
{
    var ret = true;
    var flg = data.match(/^((([01]?\d{1,2})|(2[0-4]\d)|(25[0-5]))\.){3}(([01]?\d{1,2})|(2[0-4]\d)|(25[0-5]))$/);
...
function validatePrimaryHost(data)
{
	...
	if(data == null || data.length == 0) {
		...
		//ï¿½ï¿½ï¿½pï¿½pï¿½ï¿½/ï¿½gï¿½pï¿½Â”\ï¿½Lï¿½ï¿½ï¿½`ï¿½Fï¿½bï¿½N
		if (!checkADHostNameCtrlChar(data) && !checkIpv4AddressFormat(data)) {
